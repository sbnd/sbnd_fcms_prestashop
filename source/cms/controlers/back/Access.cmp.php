...
class Access extends CmsComponent implements BuilderComponentLoginInterface{
	...
	function main(){
		...
		$this->shopFolder = CMS_SETTINGS::init()->get('SHOP_FOLDER');
		$this->shopAdminFolder = CMS_SETTINGS::init()->get('SHOP_ADMIN_FOLDER');
		
		
		if(!$this->shopFolder){
			$this->shopFolder = 'shop';
		}
		if(!$this->shopAdminFolder){
			$this->shopAdminFolder = 'cp';
		}
		...
		if($this->run_mode == 0){
			...
			$this->setField('email', array(
				'text' => BASIC_LANGUAGE::init()->get('username_or_password'),
				'perm' => '*',
				'messages' => array(
					2 => BASIC_LANGUAGE::init()->get('invalid_email_format'),
					3 => BASIC_LANGUAGE::init()->get('not_existing_email'),
	...
	function ActionSave(){
		if($this->run_mode == 0){
			if(BASIC_USERS::init()->login(
				...
				$this->loginPrestashop($this->dataBuffer['email'], $this->dataBuffer['pass']);
			...
			$this->forgotTemplateVars(array(
				...
				'active_link' => BASIC::init()->ini_get('root_virtual').'cp/?'.$this->active_var_name,
				'active_code_link' => BASIC::init()->ini_get('root_virtual').'cp/?code='.$active_code.'&'.$this->active_var_name,
	...
	function validator(){
		if($this->run_mode == 0){
			...
			if(!$this->userdata = BASIC_SQL::init()->read_exec(" SELECT * FROM `".BASIC_USERS::init()->db_table."` WHERE 
				...
				`".BASIC_USERS::init()->name_column."` = '".$this->getDataBuffer('email')."' 
			", true)){
	...
	private function loginPrestashop($email, $passwd) {
		
		$_SERVER['REQUEST_URI'] = str_replace('cp/index.php', $this->shopFolder.'/index.php?controller=AdminLogin', $_SERVER['REQUEST_URI']);
		$_SERVER['PHP_SELF'] = str_replace('cp/index.php', $this->shopFolder.'/index.php', $_SERVER['PHP_SELF']);
		...
		chdir(BASIC::init()->root().'/'.$this->shopFolder.'/'.$this->shopAdminFolder.'/');
		
		define('_PS_ADMIN_DIR_', getcwd());
		require(BASIC::init()->root().'/'.$this->shopFolder.'/config/config.inc.php');
		require(BASIC::init()->root().'/'.$this->shopFolder.'/'.$this->shopAdminFolder.'/functions.php');
		
		Dispatcher::getInstance()->dispatch();
		
		//if there is no employee user for the shop -> redirect
		if (empty($employee->id)) {
			BASIC_SESSION::init()->un('is_shop_admin');
			BASIC_URL::init()->redirect(BASIC_SESSION::init()->get($this->sess_var_for_last_path));
		} else {
			BASIC_SESSION::init()->set('is_shop_admin', 1);
		}
		
	}
	
	private function logoutPrestashop() {
			
		$_SERVER['REQUEST_URI'] = str_replace('cp/?logout=1', $this->shopFolder.'/index.php?logout', $_SERVER['REQUEST_URI']);
		$_SERVER['PHP_SELF'] = str_replace('cp/index.php', $this->shopFolder.'/index.php', $_SERVER['PHP_SELF']);
